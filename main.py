...
def text_steno():
    ...
    win = Toplevel(master=root, bg='#C1C1CD')
    win.title('Text Steganography')
    win.geometry('480x410')
    win_label = Label(win, text='Text -Steganography', font=cas_big, bg='#000000', fg='#FFFFEB')
    win_label.place(x=40, y=4)
    ...
    def encode():
        ...
        size_label = Label(win, text='Select File:', font=cas, bg='#A68064', fg='#f20713')
        ...
        se_bu = Button(win, text='Browse', bg='#A68064', font=cas, command=browse, relief='ridge')
    ...
    def decode():
        ...
        global file_loc
        dec = Toplevel(master=root, bg='#CDCDC8')
        dec.title('Text Steno-DECODE')
        dec.geometry('480x250')
        dec_label = Label(dec, text='Text -Steganography', font=cas_big, bg='#c0ed98', fg='#1046b3')
        ...
        def forgotten():
            ...
            check2 = Button(foo, text='Check & Retrieve', font=cas, bg='#A68064', fg='#2200fc', command=check)
        ...
        decode_main = Button(dec, text='Decode', relief='ridge', bg='#A68064', font=cas, command=work)
        ...
        exit_dec = Button(dec, text='Exit', bg='#A68064', font=cas, relief='ridge',
                          command=dec.destroy)
        exit_dec.place(x=360, y=200)
        ho.CreateToolTip(exit_dec, 'Closes the window')
        dec.bind('<Return>', work)

    encoding = Button(win, text='Encode data', command=encode, relief='ridge', font=cas, bg='#A68064')
    encoding.place(x=40, y=360)
    ho.CreateToolTip(encoding, 'Encoding data function')
    decoding = Button(win, text="Decode data", command=decode, relief='ridge', font=cas, bg='#A68064')
    decoding.place(x=180, y=360)
    ho.CreateToolTip(decoding, "Decoding data function")
    exit_win = Button(win, text='Exit', bg='#A68064', font=cas, relief='ridge', command=win.destroy)
...
def image_steno():
    ...
    im_lb = Label(img_win, text='Image -Steganography', bg='#000000', fg='#FFFFFA', font=cas_big)
    im_lb.place(x=10, y=10)

    def em_img():
        """Image stenography functions"""
        global file, mess
        select_lb = Label(img_win, text='Select File:', font=cas, bg='#A68064', fg='#000000')
        ...
        se_bu = Button(img_win, text='Browse', bg='#FFFFE4', font=cas, command=browse, relief='ridge')
        ...
        def pan():
            ...
            bm = Button(message, text='Done(Ctrl+b)', command=click, relief='flat', bg='#A68064', font=cas)
            bm.pack(side=BOTTOM, fill=BOTH)
            ho.CreateToolTip(bm, 'This accepts the\nmessage you entered\nand encodes it.')
            message.bind('<Control-b>', click)

        b = Button(img_win, command=pan, text='Enter Message', font=cas, bg='#A68064')
        b.place(x=10, y=100)
        ...
        main_bu = Button(img_win, text='Embed Message', bg='#A68064', font=cas, command=done)
    ...
    def ex_img():
        ...
        ex_win.title('Image Stegano-EXTRACT')
        ex_win.geometry('515x310')
        ex_lb = Label(ex_win, text='Image -Steganography', bg='#000000', fg='#FFFFFA', font=cas_big)
        ex_lb.place(x=10, y=10)
        file_lb = Label(ex_win, text='Select File:', font=cas, bg='#A68064', fg='#000000')
        ...
        se_bu = Button(ex_win, text='Browse', bg='#A68064', font=cas, command=browse, relief='ridge')
        se_bu.place(x=450, y=70)
        ho.CreateToolTip(se_bu, 'Browse thorough &\nselect the file')
        ...
        def extract_data(event=None):
            ...
            suc_lb = Label(ex_win, text='Hidden message is:', font=cas, fg='#f50c81', bg='#A68064').place(x=6, y=130)
            sh = st.ScrolledText(ex_win, width=60, height=7, font=cas)
            sh.place(x=8, y=155)
            sh.insert(INSERT, dat)
            sh.config(state=DISABLED)

        ex_bu = Button(ex_win, text='Extract Message', bg='#A68064', font=cas, command=extract_data)
    ...
    bu_en = Button(img_win, text='Embed', font=cas, bg='#A68064', fg='#000000', command=em_img)
    bu_en.place(x=70, y=220)
    ho.CreateToolTip(bu_en, 'Embeds data in image file')
    bu_ex = Button(img_win, text='Extract', font=cas, bg='#A68064', fg='#000000', command=ex_img)
    bu_ex.place(x=260, y=220)
    ho.CreateToolTip(bu_ex, 'Extracts data from image file')
    qubu = Button(img_win, text='Exit', font=cas, bg='#A68064', fg='#000000', command=img_win.destroy)
...
def audio_steno():
    ...
    aud_win = Toplevel(master=root, bg='#000000')
    aud_win.title('Audio Steno')
    aud_win.geometry('515x260')
    au_lb = Label(aud_win, text='Audio -Steganography', bg='#000000', fg='#FFFFF8', font=cas_big)
    au_lb.place(x=10, y=10)

    def em_aud():
        """Audio steno's embedding function"""
        global file, mess
        select_lb = Label(aud_win, text='Select File:', font=cas, bg='#A68064', fg='#fa0505')
        select_lb.place(x=5, y=50)
        ...
        se_bu = Button(aud_win, text='Browse', bg='#A68064', font=cas, command=browse, relief='ridge')
        se_bu.place(x=450, y=70)
        ho.CreateToolTip(se_bu, 'Browse thorough &\nselect the file')
        ...
        b = Button(aud_win, command=pan, text='Enter Message', font=cas, bg='#DEDEB8')
        b.place(x=10, y=100)
        ho.CreateToolTip(b, 'Opens a prompt where you can enter message')
        ...
        main_bu = Button(aud_win, text='Embed Message', bg='#DEDEB8', font=cas, command=done)
        main_bu.place(x=10, y=130)
        ho.CreateToolTip(main_bu, 'Checks everything and embeds your data')
    ...
    def ex_aud():
        ...
        global ex_file
        ex_win = Toplevel(root, bg='#000000')
        ex_win.title('Audio Stegano-EXTRACT')
        ex_win.geometry('515x310')
        ex_lb = Label(ex_win, text='Audio -Steganography[EXTRACT]', bg='#000000', fg='#FFFFF8', font=cas_big)
        ex_lb.place(x=10, y=10)
        file_lb = Label(ex_win, text='Select File:', font=cas, bg='#A68064', fg='#000000')
        file_lb.place(x=5, y=50)
        file_ex = Entry(ex_win, width=55, font=cas, relief='ridge')
        ...
        qu_bu = Button(ex_win, text='Exit', font=cas, bg='#A68064', fg='#000000', command=ex_win.destroy)
        qu_bu.place(x=467, y=278)
        ho.CreateToolTip(qu_bu, 'Exits window')

    bu_en = Button(aud_win, text='Embed', font=cas, bg='#A68064', fg='#000000', command=em_aud)
    bu_en.place(x=70, y=220)
    ho.CreateToolTip(bu_en, 'Embeds data in audio file')
    bu_ex = Button(aud_win, text='Extract', font=cas, bg='#A68064', fg='#000000', command=ex_aud)
    bu_ex.place(x=230, y=220)
    ho.CreateToolTip(bu_ex, 'Extracts data from audio file')
    qubu = Button(aud_win, text='Exit', font=cas, bg='#A68064', fg='#000000', command=aud_win.destroy)
    qubu.place(x=410, y=220)
    ho.CreateToolTip(qubu, 'Exits window')
...
lb = Label(root, text="STEGANOGRAPH\n 1.0", font=('Showcard Gothic', 20), bg='#C1C1CD', fg='#000000')
lb.place(x=90, y=20)

text = Button(root, text='Text', relief='flat', bg='#A68064', command=text_steno, font=cas)
text.place(x=56, y=250)
ho.CreateToolTip(text, 'Click here\nto hide your\ndata in a text file')

image = Button(root, text='Image', relief='flat', bg='#A68064', command=image_steno, font=cas)
image.place(x=156, y=250)
ho.CreateToolTip(image, 'Click here\nto hide your\ndata in an image file')

audio = Button(root, text='Audio', relief='flat', bg='#A68064', command=audio_steno, font=cas)
